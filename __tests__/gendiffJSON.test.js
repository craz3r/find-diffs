...
test('diffs between json', () => {
  ...
  const res = fs.readFileSync('__tests__/__fixtures__/resultJSON.txt', 'utf-8');

  expect(gendiff(path1, path2, 'json')).toBe(res);
});
...
test('diffs between ini', () => {
  ...
  expect(gendiff(path1, path2, 'json')).toEqual(res);
});
